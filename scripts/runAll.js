...
const CATEGORY_RULES = [
  // ★ 생활용품을 최우선으로 — 용량(ml/L) 표기 있는 세제·세정제가 음료로 오분류되는 문제 방지
  ['생활용품', /휴지|롤휴지|롤티슈|미용티슈|화장지|키친타월|물티슈|샴푸|린스|트리트먼트|치약|칫솔|초극세모|세제|섬유유연제|섬유탈취|세탁|탈취|살균|세정|방향|향균|페브리즈|다우니|피죤|퍼실|테크|비트|스파크|홈스타|슈가버블|주방세제|주방용|마스크|KF94|건전지|우산|수세미|면도기|반창고|생리대|라이너|오버나이트|탐폰|팬티라이너|기저귀|배변패드|왁스|쏘피\)|좋은\)|도루코|핸드워시|바디워시|클렌징폼|클렌징워터|로션|선크림|선스틱|에센스|아이크림|토너|마스크팩|스팟패치|콘돔|염색제|퍼머|컨디션환|컨디션스틱|\d+롤(?!케이크)|순면|유기농면|오가닛|라엘|디어스킨|예지\)|울날|슬날|빨래|섬유린스|방충|깨끗한나라|크리넥스|코디\)|모나리자/],
  ['기타', /도시락김/],
  ...
  ['음료', /콜라|사이다|주스|에이드|음료|워터|생수|우유|커피|라떼|탄산|이온음료|쉐이크|스무디|식혜|에너지드링크|핫식스|몬스터|레드불|비타500|게토레이|파워에이드|토레타|데미소다|닥터유|컵커피|바리스타|스프라이트|환타|밀키스|암바사|콤부차|아메리카노|티즐|홍차|녹차|이프로|맥콜|아이스티/],
];
